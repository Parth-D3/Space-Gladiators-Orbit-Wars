...
def cheap_enemy_pressure(obs, cache, *, horizon: float, player_id: int,
                         inbound_enemy: Tensor | None = None,
                         inbound_friendly: Tensor | None = None) -> Tensor:
    P = int(obs.P)
    device = obs.device
    ...
    if inbound_enemy is not None:
        pressure = pressure + inbound_enemy.to(dtype)
    if inbound_friendly is not None:
        pressure = pressure - inbound_friendly.to(dtype)
    return pressure.clamp(min=0.0)


def inbound_fleet_pressure(garrison_status, *, player_id: int, horizon: int) -> tuple[Tensor, Tensor]:
    arr = garrison_status.arrivals_by_owner
    device = garrison_status.ships.device
    dtype = garrison_status.ships.dtype
    P = int(garrison_status.ships.shape[0])
    if arr is None:
        return torch.zeros(P, dtype=dtype, device=device), torch.zeros(P, dtype=dtype, device=device)
    H_axis = int(arr.shape[1]) - 1
    H_eff = min(max(int(horizon), 1), H_axis)
    pid = int(player_id)
    A = int(arr.shape[-1])
    arrivals = arr[:, 1:H_eff + 1, :]
    k = torch.arange(1, H_eff + 1, device=device, dtype=dtype)
    decay = (1.0 - (k - 1) / float(H_eff)).view(1, H_eff, 1)
    friendly = (arrivals[:, :, pid] * decay.squeeze(-1)).sum(dim=1)
    if A <= 1:
        enemy = torch.zeros(P, dtype=dtype, device=device)
    else:
        others = torch.ones(A, dtype=torch.bool, device=device)
        others[pid] = False
        enemy_arr = arrivals[:, :, others]
        enemy = (enemy_arr.sum(dim=-1) * decay.squeeze(-1)).sum(dim=1)
...
def plan_lite_waves(
    *,
    ...
    wave_entries, leftover, _ = _greedy_select(
    ...
    if not bool(config.enable_regroup):
        return wave_entries
    enemy_inbound, friendly_inbound = inbound_fleet_pressure(
        garrison_status, player_id=pid, horizon=int(K_eta),
    )
    enemy_mass = cheap_enemy_pressure(
        obs, cache, horizon=float(K_eta), player_id=pid,
        inbound_enemy=enemy_inbound, inbound_friendly=friendly_inbound,
    )
