...
def overflow_attack(aux, cap=300, send_frac=0.5):
    ...
    Target order: CLOSEST first, then weakest garrison on ties. Tries
    candidates in order until one has a clear, sun-safe lane.
    ...
    for p in rows:
        ...
        def order_key(t):
            # Priority: closest first, then weakest garrison.
            # Ignore static/moving distinction — both are valid targets.
            d = math.hypot(t["x"] - p["x"], t["y"] - p["y"])
            return (d, t["ships"])  # sort by distance, then by garrison
